...
func TestUnescapedSingleQuote(t *testing.T) {
	...
	input := "Description eq 'ab'c'"

	expect := []*Token{
		{Value: "Description", Type: FilterTokenLiteral},
		{Value: "eq", Type: FilterTokenLogical},
		{Value: "'ab'", Type: FilterTokenString},
		{Value: "c", Type: FilterTokenLiteral},
	}
	output, err := tokenizer.Tokenize(input)
	if err == nil {
...
func TestDoubleSingleQuote(t *testing.T) {
	...
	expect := []*Token{
		...
		// so after the tokenization it needs to be one single quote.
		{Value: "'ab'c'", Type: FilterTokenString},
	}
	output, err := tokenizer.Tokenize(input)
	if err != nil {
		t.Error(err)
...
func TestEscapedSingleQuoteInsideTokenString(t *testing.T) {
	tokenizer := FilterTokenizer()
	// http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part2-url-conventions.html#sec_URLSyntax
	// Test single quotes escaped as %27.
	input := "Description eq 'O%27%27Neil'"

	expect := []*Token{
		{Value: "Description", Type: FilterTokenLiteral},
		{Value: "eq", Type: FilterTokenLogical},
		// Percent-encoded characters must be decoded.
		{Value: "'O'Neil'", Type: FilterTokenString},
	}
	output, err := tokenizer.Tokenize(input)
	if err != nil {
		t.Error(err)
